...
import { auth, getUserGames, acceptGameInvitation, rejectGameInvitation, startGame, isPlayerInActiveGame } from "../firebase";
...
import { BellAlertIcon, CheckCircleIcon, XCircleIcon } from "@heroicons/react/24/solid";
...
const GamesList: React.FC<GamesListProps> = ({ refreshNotifications }) => {
  ...
  const refreshGames = async () => {
    ...
    try {
      ...
      if (auth.currentUser) {
        const activeGameStatus = await isPlayerInActiveGame(auth.currentUser.uid);
      ...
      const profiles = (await Promise.all(profilePromises)).filter(Boolean) as { uid: string; profile: UserProfile }[];
      ...
      const newInvites = userGames.filter(game => 
  ...
  const handleAcceptInvitation = async (gameId: string) => {
    try {
      ...
      if (isInActiveGame) {
        setError("You cannot accept this invitation because you're already in an active game");
        return;
      }
  ...
  const invitationsReceived = games.filter(game => 
    game.status === "invited" && 
    game.opponent === auth.currentUser?.uid
  );
  
  const pendingInvitations = games.filter(game => 
    game.status === "invited" && 
    game.createdBy === auth.currentUser?.uid
  );
  ...
  const waitingConfirmation = games.filter(game => 
    game.status === "waiting_confirmation" && 
    game.confirmedBy === auth.currentUser?.uid
  );
  
  const otherWaitingGames = games.filter(game => 
    game.status === "waiting_confirmation" && 
    game.confirmedBy !== auth.currentUser?.uid
  );
  
  // All games requiring action are those that are active or waiting confirmation
  const actionRequiredGames = [...activeGames, ...waitingConfirmation, ...otherWaitingGames];
  ...
  return (
    <div className="p-6 max-w-4xl mx-auto text-zinc-900 dark:text-white">
      <div className="flex justify-between items-center mb-6">
          ...
          <h1 className="text-2xl font-bold flex items-center">
            ...
            {hasNewInvitations && (
              <span className="ml-3 inline-flex items-center">
                ...
                <span className="ml-1 text-sm font-medium text-amber-600">New Invitations!</span>
              </span>
            )}
          </h1>
          <button 
            ...
            className="ml-3 p-1 rounded-full text-gray-500 hover:bg-gray-100 hover:text-gray-700 dark:text-gray-400 dark:hover:bg-gray-800 dark:hover:text-gray-300"
        ...
        <Link
          to="/create-game"
          className={`px-4 py-2 ${isInActiveGame 
      ...
      {isInActiveGame && (
        <div className="mb-6 p-4 bg-blue-100 border border-blue-400 text-blue-800 rounded-md">
          ...
          <p>You must complete your active game before creating or joining a new one.</p>
        </div>
      )}
      ...
      {games.length === 0 ? (
        ...
        <div className="space-y-6">
              ...
              <div className="bg-white dark:bg-zinc-800 rounded-lg shadow overflow-hidden border-2 border-amber-400 dark:border-amber-500">
                <ul className="divide-y divide-gray-200 dark:divide-zinc-700">
                  {invitationsReceived.map(game => (
                    <li key={game.id} className="hover:bg-amber-50 dark:hover:bg-amber-900/20">
                      <div className="p-4">
                        <div className="flex justify-between items-center mb-4">
                          <div>
                            <p className="font-bold text-lg">Invitation from {getOpponentName(game)}</p>
                            <p className="text-sm text-gray-500 dark:text-zinc-400">
                              {formatDate(game.createdAt)} • {game.settings.gameMode} • {game.settings.pointsToWin} points
                            </p>
                        ...
                        <div className="flex justify-end space-x-3">
                          <button
                            onClick={() => handleRejectInvitation(game.id || "")}
                            disabled={actionInProgress === game.id || isInActiveGame}
                          ...
                          >
                            {actionInProgress === game.id ? "Processing..." : "Decline"}
                          </button>
                          <button
                        ...
                        {isInActiveGame && !actionInProgress && (
                          <div className="mt-2 p-2 bg-amber-50 border border-amber-200 text-amber-800 rounded-md dark:bg-amber-900/20 dark:border-amber-800 dark:text-amber-200">
                            <p className="text-sm">You must complete your active game before accepting a new invitation.</p>
                            <Link 
                              to={`/games`} 
              ...
              <div className="bg-white dark:bg-zinc-800 rounded-lg shadow overflow-hidden">
                <ul className="divide-y divide-gray-200 dark:divide-zinc-700">
                  {pendingInvitations.map(game => (
                    <li key={game.id} className="hover:bg-gray-50 dark:hover:bg-zinc-700">
                      <Link to={`/game/${game.id}`} className="block p-4">
                        <div className="flex justify-between items-center">
                          <div>
                            <div className="flex items-center">
                              <p className="font-medium">Invitation to {getOpponentName(game)}</p>
                    ...
                    <li key={game.id} className="hover:bg-gray-50 dark:hover:bg-zinc-700">
                      <div className="p-4">
                        <div className="flex justify-between items-center mb-2">
                          <div>
                            <p className="font-medium">Game with {getOpponentName(game)}</p>
                            <p className="text-sm text-gray-500 dark:text-zinc-400">
                              {formatDate(game.createdAt)} • {game.settings.gameMode} • {game.settings.pointsToWin} points
                            </p>
                          </div>
                        ...
                        {game.status === "accepted" && (
                          <div className="flex justify-end">
                            ...
                            >
                              {actionInProgress === game.id ? "Starting..." : "Start Game"}
                            </button>
                          </div>
                        )}
                        
                        {(game.status === "in_progress" || game.status === "waiting_confirmation") && (
                          <div className="flex justify-end">
                            <Link 
                              to={`/game/${game.id}`}
                              className="px-3 py-1.5 text-sm font-medium rounded bg-blue-600 text-white hover:bg-blue-700"
                            >
                              {game.status === "waiting_confirmation" ? "View Game" : "Continue Game"}
                            </Link>
                          </div>
                        )}
                      </div>
          ...
          {false && (
            <div>
              <h2 className="text-xl font-semibold mb-3">Games Waiting Your Confirmation</h2>
              <div className="bg-white dark:bg-zinc-800 rounded-lg shadow overflow-hidden">
                <ul className="divide-y divide-gray-200 dark:divide-zinc-700">
          ...
          {false && (
            <div>
              <h2 className="text-xl font-semibold mb-3">Waiting for Opponent Confirmation</h2>
              <div className="bg-white dark:bg-zinc-800 rounded-lg shadow overflow-hidden">
                <ul className="divide-y divide-gray-200 dark:divide-zinc-700">
              ...
              <div className="bg-white dark:bg-zinc-800 rounded-lg shadow overflow-hidden">
                <ul className="divide-y divide-gray-200 dark:divide-zinc-700">
                  {completedGames.map(game => {
                    const currentUserWon = game.winner === auth.currentUser?.uid;
                    return (
                      <li key={game.id} className="hover:bg-gray-50 dark:hover:bg-zinc-700">
                        <Link to={`/game/${game.id}`} className="block p-4">
                          <div className="flex justify-between items-center">
                            <div>
                              ...
                              {game.scores && (
                                <p className="text-sm mt-1">
                                  Score: {game.scores.creator} - {game.scores.opponent}
                                </p>
                              )}
                            </div>
