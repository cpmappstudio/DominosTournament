          ...
          <div className="flex-grow">
            ...
            <div className="mt-3">
              <span className="inline-block px-3 py-1 rounded-full text-sm font-medium bg-blue-100 text-blue-800 dark:bg-blue-900 dark:text-blue-100">
                {userProfile?.stats ? calculateTitle(userProfile.stats.gamesWon) : "New Player"}
              </span>
            </div>
          </div>
        </div>
      ...
      <div className="bg-white dark:bg-zinc-800 rounded-lg shadow p-6 mb-8">
        ...
        <div className="grid grid-cols-2 md:grid-cols-3 gap-4">
          <div className="p-4 bg-gray-50 dark:bg-zinc-700 rounded-lg text-center">
            ...
            <p className="text-2xl font-bold">{userProfile?.stats?.gamesPlayed || 0}</p>
          </div>
          
          <div className="p-4 bg-gray-50 dark:bg-zinc-700 rounded-lg text-center">
            <p className="text-sm text-zinc-600 dark:text-zinc-400">Games Won</p>
            <p className="text-2xl font-bold">{userProfile?.stats?.gamesWon || 0}</p>
          </div>
          
          <div className="p-4 bg-gray-50 dark:bg-zinc-700 rounded-lg text-center">
            <p className="text-sm text-zinc-600 dark:text-zinc-400">Total Points</p>
            <p className="text-2xl font-bold">{userProfile?.stats?.totalPoints || 0}</p>
          </div>
        </div>
      </div>
      ...
      <div className="bg-white dark:bg-zinc-800 rounded-lg shadow p-6">
        ...
        <div className="overflow-x-auto">
          <table className="min-w-full divide-y divide-gray-200 dark:divide-zinc-700">
            <thead>
              <tr>
                <th className="px-4 py-3 text-left text-xs font-medium text-gray-500 dark:text-zinc-400 uppercase tracking-wider">Date</th>
                <th className="px-4 py-3 text-left text-xs font-medium text-gray-500 dark:text-zinc-400 uppercase tracking-wider">Opponent</th>
                <th className="px-4 py-3 text-left text-xs font-medium text-gray-500 dark:text-zinc-400 uppercase tracking-wider">Type</th>
                <th className="px-4 py-3 text-left text-xs font-medium text-gray-500 dark:text-zinc-400 uppercase tracking-wider">Result</th>
                <th className="px-4 py-3 text-left text-xs font-medium text-gray-500 dark:text-zinc-400 uppercase tracking-wider">Score</th>
              </tr>
            </thead>
            <tbody className="divide-y divide-gray-200 dark:divide-zinc-700">
              {recentGames.length > 0 ? recentGames.map((game) => (
                <tr key={game.id}>
                  <td className="px-4 py-3 whitespace-nowrap text-sm">{formatDate(game.date.toString())}</td>
                  <td className="px-4 py-3 whitespace-nowrap text-sm">{game.opponentName}</td>
                  <td className="px-4 py-3 whitespace-nowrap text-sm capitalize">{game.gameMode}</td>
